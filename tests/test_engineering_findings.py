...
from fakes.fake_provider import ScriptedAnswer
...
def _install_fake_agent(main_module, monkeypatch, chunks: list[str], tool_calls: list[tuple[str, dict]] | None = None):
    ...
    at all -- enough to exercise the propose-resolution wiring itself
    ...
    propose-resolution call each test actually exercises.
    """
...
def test_propose_resolution_is_illegal_outside_action_required(monkeypatch, tmp_path) -> None:
    _configure_env(monkeypatch, tmp_path)
    import app.main as main_module
    ...
    with TestClient(main_module.app) as client:
        ...
        response = client.post(f"/api/v1/findings/{finding_id}/propose-resolution")

    assert response.status_code == 409


def test_propose_resolution_is_illegal_for_a_non_dimension_mismatch_finding_type(monkeypatch, tmp_path) -> None:
    _configure_env(monkeypatch, tmp_path)
    import app.main as main_module

    with TestClient(main_module.app) as client:
        client.post("/api/v1/chat", json={"question": RECONCILIATION_QUESTION})
        finding_id = next(item["finding_id"] for item in client.get("/api/v1/findings").json() if item["tag"] == "D04")  # missing_in_pdf
...
def test_propose_resolution_produces_a_real_proposal_with_real_citations(monkeypatch, tmp_path) -> None:
    """Owner decision, 2026-09-18 (OD-52): the agent investigates and
    proposes a value -- this locks in that `propose_finding_resolution`
    actually calls V2's real tool-calling loop against the real IFC/PDF
    fixtures (a real `get_element_properties` call executes for real; only
    the model's own two responses are scripted), not a stub, and that the
    resulting citations trace to real evidence, not invented ones.
    """
    _configure_env(monkeypatch, tmp_path)
    import app.main as main_module
    ...
    proposal = response.json()["pending_proposal"]
...
def test_approve_proposal_history_snapshot_survives_a_later_overwriting_proposal(monkeypatch, tmp_path) -> None:
    ...
    by a later `propose-resolution` call that replaces the finding's live
    ...
    with TestClient(main_module.app) as client:
        fake = _install_fake_agent(main_module, monkeypatch, ["The IFC value 1.75 m is correct."])
        client.post("/api/v1/chat", json={"question": RECONCILIATION_QUESTION})
        finding_id = next(item["finding_id"] for item in client.get("/api/v1/findings").json() if item["tag"] == "W02")
        _walk_to_action_required(client, finding_id)

        client.post(f"/api/v1/findings/{finding_id}/propose-resolution")
        ...
        # nothing was actually fixed) so a *second* propose-resolution call
        # is legal again, then confirm it doesn't touch the first approval's
        ...
        second_propose = client.post(f"/api/v1/findings/{finding_id}/propose-resolution")
        assert second_propose.status_code == 200
        assert second_propose.json()["pending_proposal"]["rationale"] == "A completely different, later conclusion."
