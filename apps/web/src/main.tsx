...
function App() {
  ...
  const [drawingEvidence, setDrawingEvidence] = useState<{ bbox?: number[]; board?: string; field?: string } | null>(null);
  ...
  function openCitation(citation: Citation) {
    ...
    if (citation.source_type === "pdf") {
      setTab("drawing");
      setDrawingEvidence({ bbox: citation.locator.bbox, board: citation.locator.board, field: citation.locator.field });
  ...
  return <main>
    ...
    <section className="workspace">
      <aside className="viewer"><div className="tabs"><button className={tab === "bim" ? "active" : ""} onClick={() => setTab("bim")}>BIM Model</button><button className={tab === "drawing" ? "active" : ""} onClick={() => setTab("drawing")}>Drawing</button><button className={tab === "snapshot" ? "active" : ""} onClick={() => setTab("snapshot")}>Viewer Snapshot</button></div>
        ...
        {tab === "drawing" && <section className="drawing"><h2>Engineering Drawing</h2><p>{metadata?.pdf_file || "synthetic schedule"} · page 1</p><div className="drawing-toolbar"><button type="button" onClick={() => adjustDrawingZoom(drawingZoom - .25)}>−</button><span>{Math.round(drawingZoom * 100)}%</span><button type="button" onClick={() => adjustDrawingZoom(drawingZoom + .25)}>+</button><button type="button" onClick={() => setDrawingZoom(1)}>Fit page</button><button type="button" onClick={() => setDrawingZoom(1.15)}>Fit width</button><button type="button" onClick={() => setDrawingZoom(1)}>100%</button>{drawingEvidence && <button type="button" onClick={() => { setDrawingEvidence(null); setDrawingZoom(1); }}>Clear evidence focus</button>}</div><div className="drawing-stage" aria-label="Zoomable engineering drawing. Pinch to zoom; two-finger scroll pans." onWheel={onDrawingWheel}><div className="drawing-page" style={{ transform: `scale(${drawingZoom})` }}><img src="/api/v1/project/pdf/pages/1.png" alt="Engineering load schedule page 1" />{drawingEvidence?.bbox && <div ref={drawingEvidenceRef} className="drawing-evidence-box" style={{ left: `${(drawingEvidence.bbox[0] / 1191) * 100}%`, top: `${(drawingEvidence.bbox[1] / 842) * 100}%`, width: `${((drawingEvidence.bbox[2] - drawingEvidence.bbox[0]) / 1191) * 100}%`, height: `${((drawingEvidence.bbox[3] - drawingEvidence.bbox[1]) / 842) * 100}%` }} title={`${drawingEvidence.board || "PDF evidence"}${drawingEvidence.field ? ` · ${drawingEvidence.field}` : ""}`} />}</div></div><p className="empty">{drawingEvidence ? `Focused evidence: ${drawingEvidence.board || "drawing region"}${drawingEvidence.field ? ` · ${drawingEvidence.field}` : ""}.` : "Pinch to zoom and use two-finger scrolling to pan; citations focus a cited drawing region."}</p></section>}
