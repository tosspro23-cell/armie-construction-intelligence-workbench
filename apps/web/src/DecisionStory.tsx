...
// bottom story instead: Question -> Plan -> Execution -> Evidence ->
// Verification -> Result, with the full raw trace demoted to one
// collapsed section at the end for anyone who wants to dig further.
...
function auditStage(event: TraceEvent): typeof AUDIT_STAGES[number] {
...
export function DecisionStory({ latest, trace, projectId, onOpenCitation }: {
  ...
  const stageGroups = AUDIT_STAGES.map((stage) => ({ stage, events: trace.filter((event) => auditStage(event) === stage) }));
  ...
  return <section className="decision-story">
    ...
    <ol className="story-steps">
      ...
      <li className="story-step">
        ...
        {retrievalEvent && <div className="retrieval-evidence">
          <strong>Azure AI Search retrieval considered</strong>
          <span className="empty">Relevance threshold: {retrievalEvent.payload.relevance_threshold} · a document is only named in the answer if its score clears it</span>
    ...
    <details className="raw-trace">
      <summary>Full technical trace ({trace.length} events)</summary>
      {stageGroups.map(({ stage, events }) => events.length === 0 ? null : <details className="audit-stage" key={stage}>
        <summary>{stage} ({events.length})</summary>
        <ol>{events.map((event) => <li key={event.id}><strong>{event.summary}</strong>{event.actual_model && <small>Model: {event.actual_provider}/{event.actual_model}</small>}<details><summary>Payload</summary><pre>{JSON.stringify(event.payload, null, 2)}</pre></details></li>)}</ol>
      </details>)}
      <details className="audit-stage">
